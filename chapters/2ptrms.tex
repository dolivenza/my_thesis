...
The ions will predominantly collide with the buffer gas molecules. \autoref{eq:mfp} gives the expression for the kinetic mean free path, \acrshort{mfp}, defined as the average distance travelled between collisions, if the colliding particles are considered hard-spheres \cite{hirschfelder1954molecular}.
%
\begin{equation}
...
On the other hand, the transient experiments are stored in %raw files called
.lst files.
 These %are binary
...
%\subsection{Code for data analysis}
 With a PTR-MS instrument, a lot of data can be acquired quite quickly so basic programming knowledge comes in handy when dealing with these big datasets.
...
Analysing data from fast switching experiments can be tedious if needed to be done working manually with each file. %file by file.
%
...
The files that this script imports are the .lst ones and its accompanying .ini files, which contain the information of the fast switching experiment that is necessary to work with the data.
...
This issue has been addressed in many interesting ways.
%
...
%Similar aproaches to that mentioned here are taken by \citeauthor{ebejer2005rapid} \cite{ebejer2005rapid}
...
We used the LCU to study the reactions of several ketones with H$_3$O$^+$ in humid conditions \cite{malaskova2019compendium}, where this add-on was not used to calibrate the instrument but to create a steady signal of ketones samples in humid conditions. % at 100\% relative humidity at 37$^{\circ}$C.
